...
def create_tf_example(filename, encoded_jpeg, annotations):
    ...
    # Converting the data to the desired format
...
def process_tfr(filepath, data_dir):
    ...
    #logger.info('Defined destination file')
    dataset = tf.data.TFRecordDataset(filepath, compression_type='')
    for idx, data in enumerate(dataset):
        #logger.info('Itereating in data')
        ...
        #logger.info('received tfexample')
        writer.write(tf_example.SerializeToString())
        #logger.info('Write to new file')
    writer.close()
